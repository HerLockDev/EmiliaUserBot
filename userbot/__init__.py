# Copyright (C) 2021 The EmiliaProjects Company LLC.
#
# Licensed under the EmiliaProjects Public License, Version 1.c (the "License");
# you may not use this file except in compliance with the License.
#

# Thanks github.com/spechide for creating inline bot support.
# EmiliaUserBot - Erdem By - ByMisakiMey
""" UserBot hazırlanışı. """
...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        level=DEBUG,
        format="[%(asctime)s - %(levelname)s] - @EmiliaUserBot : %(message)s",
        datefmt='%d-%b-%y %H:%M:%S')
else:
    basicConfig(
        level=INFO,
...
if HEROKU_APPNAME is not None and HEROKU_APIKEY is not None:
    if EZZEC == True:
        ...
        heroku_var["UPSTREAM_REPO_URL"] = "https://github.com/EmiliaProjects/EmiliaUserBot.git"
else:
    app = None
...
UPSTREAM_REPO_URL = "https://github.com/EmiliaProjects/EmiliaUserBot.git" #if not STABILITY else https://github.com/erdewbey/EmiliaUserBot.git Eski Dostum Berce'ye Aittir.
EMERGENCY = "https://github.com/erdewbey/EmiliaUserBot.git" # Acil durrum için
# Afk mesajların iletilmesi
AFKILETME = sb(os.environ.get("AFKILETME", "True"))

# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///emilia.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@EmiliaUserBot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "| 🌃 @EmiliaUserBot Paketi |")

# Userbotu kapatmak için gruplar
BLACKLIST_CHAT = os.environ.get("BLACKLIST_CHAT", None)

if not BLACKLIST_CHAT: #Eğer ayarlanmamışsa Emilia Support grubu eklenir.
...
DEVS = 2124244679 #developer ayrıcalıkları olacak
...
with bot:
    ...
    try:
        ...
        bot(JoinChannelRequest("@EmiliaUserBot"))
        bot(JoinChannelRequest("@EmiliaSupport"))
        bot(JoinChannelRequest("@EmiliaProjects"))
        bot(JoinChannelRequest("@instasiri"))
        bot(JoinChannelRequest("@EmiliaProjectsChat"))
        bot(JoinChannelRequest("@emiliaplugin"))
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @EmiliaUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Emilia açabilirsin; Kanala bak` @EmiliaUserBot')
            else:
                await event.reply(f'`Tengri save Turks! Emilia working... `')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@EmiliaUserBot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**En Gelişmiş UserBot!** [Emilia](https://t.me/EmiliaUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@EmiliaUserBot",
                    text="""@EmiliaUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/EmiliaUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/EmiliaSupport")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/erdewbey/EmiliaUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ Hey! Benim mesajlarımı düzenlemeye kalkma! Kendine bir @EmiliaUserBot kur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"** En Gelişmiş UserBot!** [Emilia](https://t.me/EmiliaUserBot) __Çalışıyor...__\n\n**Yüklenen Modül Sayısı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
        ...
        LOGS.info(
            ...
            "Eğer bunun dışında bir sorun olduğunu düşünüyorsanız bize ulaşın t.me/EmiliaSupport."
        )
